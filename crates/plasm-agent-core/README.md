...
- `**PLASM_TRACE_SINK_URL`** — Base URL for `**POST /v1/events**` (durable `mcp_trace_segment` audit rows to the trace-sink service). When unset, live SSE still streams from in-memory [`trace_hub`](src/trace_hub.rs), but nothing is persisted; a **one-time** `tracing::warn!` is emitted per process on first attempted ingest.
- `**PLASM_TRACE_SINK_READ_URL` / `PLASM_TRACE_SINK_URL`** — HTTP base for `GET /v1/traces` and `GET /v1/traces/{id}` (durable list/detail; read URL wins when set). If neither is set, `**PLASM_TRACE_ARCHIVE_DIR**` can provide **local** OSS self-host list/detail; otherwise tenant-scoped history returns **503** (no silent empty history).
- `**PLASM_TRACE_ARCHIVE_DIR**` — On finalize, completed sessions are also written under `traces/{tenant}/{trace_id}/` for later reads (additive OSS tier; does not replace the hosted object-store + sink path when those envs are set).
- `**PLASM_RUN_ARTIFACTS_URL` / `PLASM_RUN_ARTIFACTS_DIR`** — Run snapshot storage: object-store URL (SaaS/highest precedence) or local directory when URL unset; both unset → in-memory. See [`run_artifacts`](src/run_artifacts.rs).
