...
frontmatter teaching grammar that Plasm exposes through MCP initialize \
instructions.\n\n\
```text\n{}\n```\n",
        frontmatter.trim()
    )
...
///
/// `frontmatter` must come from [`plasm_core::prompt_render::render_plasm_mcp_language_frontmatter`]
/// so grammar stays aligned with MCP initialize instructions and `.plasm/grammar.md`.
