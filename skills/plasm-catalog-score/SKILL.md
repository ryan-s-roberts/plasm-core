...
|----|-----------|---|---|---|---|---|
| A | Semantic compression | RPC-shaped — one capability per endpoint, no merges | Some merges, but still mostly RPC | Domain entities exist, relations partial | Compressed, relations cover common navigation | Compressed model with clear entity story; capability set tracks domain verbs, not paths |
