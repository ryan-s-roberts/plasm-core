# Incremental DOMAIN prompts and reducing prompt churn

!!! tip "Read this when…"

    You tune MCP prompts, federate multiple catalogs in one session, or debug “why did `e#` / `m#` change?”

**What to know first:** [Concepts](../concepts.md) (symbols `e#` / `m#` / `p#`), [MCP session reuse](mcp-session-reuse.md) (`intent`, reuse).

**Practical takeaway:** DOMAIN ships in **waves**; symbol indices are **append-only** per session so agents do not see reorder churn when new entity types appear.

---

This document describes how Plasm serves **DOMAIN** (many-shot, symbol-tuned examples) for execute sessions, and why that design **reduces prompt churn** for agents and humans.
...
Implementation: `[FocusSpec::SeedsExact](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/symbol_tuning.rs)` and `[entity_slices_for_render](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/symbol_tuning.rs)`.

### `DomainExposureSession`

A session-scoped structure in **plasm-core** (`[DomainExposureSession](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/symbol_tuning.rs)`) allocates:
...
`[ExecuteSession](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-agent-core/src/execute_session.rs)` holds:

- `prompt_text` — Cumulative DOMAIN text (wave 1 + optional `## Expanded capabilities` sections).
- `domain_exposure` — The [`DomainExposureSession`] used for both DOMAIN rendering and `[expand_expr_for_domain_session](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/symbol_tuning.rs)` (via `[expand_expr_for_session_with_optional_exposure](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/prompt_pipeline.rs)`).
...
Cardinality: **many** logical sessions per MCP **transport** (`MCP-Session-Id`); **one** active Plasm execute binding per **logical session** (see `[mcp_server.rs](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-agent-core/src/mcp_server.rs)` module docs).

## Federated sessions (multi-catalog)

A single execute session (`prompt_hash` + `session`) can expose entities from **more than one** registry row (`entry_id`) **without** merging their `[CGS](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/schema.rs)` graphs into one artifact.

- **Prompt / symbols** — `[DomainExposureSession](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/symbol_tuning.rs)` tracks which catalog each exposed entity name belongs to; DOMAIN rendering and the symbol map stay **append-only** (`e#` / `m#` / `p#` monotonic **within that session**). Headings and tables can reflect **(registry entry, entity)** so the model knows which API each block refers to.
- **Execution** — The agent keeps one `[CgsContext](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/cgs_context.rs)` per `entry_id` (backend URL, auth, and its own `CGS`). `[FederationDispatch](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/cgs_federation.rs)` maps exposed entity names to the owning context; the runtime selects HTTP origin (and typecheck graph) **per operation**, not a single merged schema.
...
- **HTTP** — Primary flow is still `POST /execute` with one `entry_id`; extending with a second catalog may use the same federate path as MCP where implemented (see `[http_execute.rs](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-agent-core/src/http_execute.rs)`).
...
- CGS template binding helpers (DOMAIN anchor / invoke id): `[crates/plasm-core/src/schema.rs](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/schema.rs)` (`template_domain_exemplar_requires_entity_anchor`, `template_invoke_requires_explicit_anchor_id`)
- Federation dispatch (multi-context, no CGS merge): `[crates/plasm-core/src/cgs_federation.rs](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/cgs_federation.rs)`
- Symbol tuning and exposure: `[crates/plasm-core/src/symbol_tuning.rs](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/symbol_tuning.rs)`
- DOMAIN rendering: `[crates/plasm-core/src/prompt_render.rs](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/prompt_render.rs)`
- Prompt pipeline: `[crates/plasm-core/src/prompt_pipeline.rs](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/prompt_pipeline.rs)`
- HTTP + expand: `[crates/plasm-agent-core/src/http_execute.rs](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-agent-core/src/http_execute.rs)`
- MCP: `[crates/plasm-agent-core/src/mcp_server.rs](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-agent-core/src/mcp_server.rs)`

## Summary

**Prompt churn** is reduced by (1) **exact** first-wave DOMAIN size, (2) **append-only** waves via `plasm_context` seed deltas, (3) **no duplicate DOMAIN** on reused opens, and (4) **monotonic** `e#`/`m#`/`p#` so earlier examples remain valid as the session grows. **Federation** adds (5) **multi-catalog** sessions without merging CGS — same monotonic symbol stream, dispatch per `[CgsContext](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-core/src/cgs_context.rs)`.