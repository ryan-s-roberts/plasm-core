...
- **Iceberg / trace sink:** Historical rows keep old `trace_id` values; new ingests use v2. `**GET /v1/traces/{id}`** detail is built only from `event_kind = mcp_trace_segment` rows (payload = canonical `[TraceEvent](https://github.com/ryan-s-roberts/plasm-core/blob/main/crates/plasm-trace/src/lib.rs)`); older audit kinds are not interpreted.
