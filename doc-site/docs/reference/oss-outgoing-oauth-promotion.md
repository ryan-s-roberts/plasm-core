...
**Do not conflate** with saas-architecture.md (hosted product; not published in OSS docs) §0.4 planes: outbound provider auth ≠ MCP transport keys ≠ execute JWT identity.
...
| Control-plane contract | private-control-plane-api.md (hosted product; not published in OSS docs) — `/internal/oauth-link/v1/*`, `/internal/outbound-secrets/v1/*` |
...
- Shared header auth for `/internal/*` lives in `plasm-agent-core` ``control_plane_http.rs``; OSS-mounted outgoing routes need a **different** gate.
